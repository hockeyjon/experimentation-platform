# ---------------------------------------------------------------------------
# DEV frontend (Phase 3 build env) — a throwaway mirror of the prod frontend,
...
#
# First run is two-phase (DNS lives at GoDaddy):
...
#
# Teardown after the Phase 3 cutover: targeted destroy of these resources
# (empty the bucket first), then delete this file — same as Phase 2.
# ---------------------------------------------------------------------------
...
resource "aws_cloudfront_distribution" "frontend_dev" {
  ...
  comment             = "Experimentation frontend (DEV / Phase 3)"
...
output "dev_acm_validation_record" {
  description = "Add this CNAME at GoDaddy to validate the dev cert."
  value = {
...
output "dev_cloudfront_distribution_id" { value = aws_cloudfront_distribution.frontend_dev.id }
output "dev_bucket_name" { value = aws_s3_bucket.frontend_dev.bucket }
